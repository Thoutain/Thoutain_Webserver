...
// 实现日志创建、写入方式的判断
// write_log函数完成写入日志文件爱着你红的具体内容，主要实现日志分级、分文件、格式化输出内容
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines, int max_queue_size){
    ...
    if (max_queue_size >= 1){
        ...
        // 创建并设置阻塞队列长度
        m_log_queue = new block_queue<string>();
        pthread_t tid;
    ...
    if (p != NULL){
        ...
        snprintf(log_full_name, 255, "%d_%02d_%02d_%s", my_tm.tm_year + 1900, my_tm.tm_mom + 1, my_tm.tm_mday, file_name);
...
void Log::write_log(int level, const char *format, ...){
    ...
    int n = snprintf(m_buf, 48, "%s-%02d-%02d %02d:%02d:%02d.%06ld %s",
